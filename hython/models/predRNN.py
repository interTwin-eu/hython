...
class PredRNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        if self.configs.conv_on_input == 1:
            ...
            self.action_conv_input1 = nn.Conv2d(
                self.action_ch,
                num_hidden[0] // 2,
                configs.filter_size,
                stride=2,
            ...
            self.action_conv_input2 = nn.Conv2d(
                num_hidden[0] // 2,
                num_hidden[0],
                configs.filter_size,
                stride=2,
    ...
    def forward(self, all_frames, mask_true):
        ...
        for t in range(self.configs.total_length - 1):
            ...
            for i in range(1, self.num_layers):
                h_t[i], c_t[i], memory, delta_c, delta_m = self.cell_list[i](
                    h_t[i - 1], h_t[i], c_t[i], memory, action
                )
        ...
        loss = (
            self.MSE_criterion(
                next_frames, all_frames[:, 1:, :, :, : next_frames.shape[4]]
            )
...
class ActionSTLSTMCell(nn.Module):
    ...
    def forward(self, x_t, a_t, h_t, c_t, m_t):
        ...
        i_x, f_x, g_x, i_xp, f_xp, g_xp, o_x = torch.split(
            x_conv, self.num_hidden, dim=1
        )
...
class ForcedSTRNN(nn.Module):
    def __init__(
        ...
        self.conv_last = nn.Conv2d(
            num_hidden[num_layers - 1], self.out_channel, kernel_size=1, bias=False
        )
        # convolution on inputs
        adapter_num_hidden = num_hidden[0]
        self.adapter = nn.Conv2d(
            adapter_num_hidden, adapter_num_hidden, kernel_size=1, bias=False
        )
    ...
    def forward(self, forcings, init_cond, static_inputs):
        ...
        c_t = list(
            torch.split(self.cell_encoder(static_inputs[:, 0]), self.num_hidden, dim=1)
        )
        ...
        for t in range(timesteps):
            ...
            h_t[0], c_t[0], memory, dc, dm = self.cell_list[0](
                x, a, h_t[0], c_t[0], memory
            )
            ...
            for i in range(1, self.num_layers):  # LOOP OVER OTHER LAYERS
                h_t[i], c_t[i], memory, dc, dm = self.cell_list[i](
                    h_t[i - 1], a, h_t[i], c_t[i], memory
                )
            ...
            for i in range(self.num_layers):
                decouple_loss.append(
                    self.calc_decouple_loss(delta_c_list[i], delta_m_list[i])
                )
