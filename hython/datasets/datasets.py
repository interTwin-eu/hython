...
class LSTMDischargeDataset(Dataset):

    def __init__(self, 
                 xd:torch.Tensor | npt.NDArray, 
                 y:torch.Tensor  | npt.NDArray, # only discharge
                 xs:torch.Tensor | npt.NDArray = None, 
                 seq_length:int = 60, 
                 create_seq:bool = False):
        """Create a dataset for training and validating LSTM-based models
    ...
    def __getitem__(self, i):
        if self.xs is not None:
            if self.create_seq:
                return self.Xd[i], self.xs, self.y[0]
            else:
                return self.Xd[i], self.xs[i], self.y[0]
        else:
            return self.Xd[i], self.y[0]